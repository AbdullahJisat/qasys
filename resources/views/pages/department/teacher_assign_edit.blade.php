        ...
        <div class="card">
            <div class="card-body">
                ...
                {{-- <form id="demo-form" data-parsley-validate="" enctype="multipart/form-data" action="{{route('department.store')}}" method="post"> --}}
                    <form action="{{route('department.assign.update',$department_teacher->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="teacher_id" class="form-control" id="">
                            ...
                            @foreach (App\Models\User::where('user_type', 'teacher')->get() as $teacher)
                                <option value="{{$teacher->id}}" {{(@$department_teacher->id == $teacher->id) ? "selected" : ""}}>{{$teacher->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="department_id[]" id="" multiple class="form-control">
                            ...
                            <option value="{{$department->id}}" {{(@(in_array($department->id, $department_teacher->departments->pluck('id')->toArray()))) ? "selected" : ""}}>{{$department->name}}({{$department->code}})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
