        ...
        <div class="card">
            <div class="card-body">
                ...
                {{-- <form id="demo-form" data-parsley-validate="" enctype="multipart/form-data" action="{{route('course.store')}}" method="post"> --}}
                    <form action="{{route('course.assign-student.update',$course_student->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="course_id[]" id="" multiple class="form-control">
                            ...
                            <option value="{{$course->id}}" {{(@(in_array($course->id, $course_student->courses->pluck('id')->toArray()))) ? "selected" : ""}}>{{$course->name}}({{$course->code}})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
